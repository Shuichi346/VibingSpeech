...
if [[ -z "${OUT_DIR:-}" || ! -d "$OUT_DIR" ]]; then
  echo "error: could not locate SwiftPM output dir for config=$CONFIG under $BUILD_DIR" >&2
  exit 1
fi
...
if [[ ! -d "$KERNELS_DIR" ]]; then
  ...
  echo "hint: run 'swift build' first to fetch dependencies" >&2
  exit 1
fi
...
if [[ -f "$OUT_METALLIB" && -f "$HASH_FILE" ]]; then
  ...
  if [[ "$CURRENT_HASH" == "$PREV_HASH" ]]; then
    echo "mlx.metallib is up to date, skipping build"
    exit 0
  fi
fi
...
echo "Compiling ${#METAL_SRCS[@]} Metal shader sources..."
