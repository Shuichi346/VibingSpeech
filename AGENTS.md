...
- For Text Processing (LLM), keep `Qwen3Chat` linked in `VibingSpeech.xcodeproj`, use `mlx-community/Qwen3.5-4B-MLX-4bit`, and rely on the chat template's `enableThinking: false` path for non-thinking mode; do not use `/think` or `/nothink` prompt switches for Qwen3.5.
