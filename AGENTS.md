...
- Keep the recording overlay compact and bottom-centered. Its 150 x 33 panel geometry lives in `VibingSpeech/Views/RecordingOverlay.swift`, and the overlay should not show phase text unless that UI direction changes explicitly. When Live Transcription is enabled, use the separate transcript pop-up above the compact panel; do not enlarge or replace the compact panel.
